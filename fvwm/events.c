...
// BEGIN HACK
extern int mod_released;
void swap_windows(FvwmWindow *prev_fw)
{
// reset hack state
    mod_released = 0;
// Disabled if reversing CirculateUp & CirculateDown in system.fvwm2rc:
//return;
// move the previous focus to the last position so mod-tab works
    if(prev_fw)
    {
        FvwmWindow *ptr = prev_fw;
// get the end of the list
        ...
        {
// unlink it
            FvwmWindow *prev = prev_fw->prev;
            FvwmWindow *next = prev_fw->next;
...
// BEGIN HACK
// store the previous focused window
    FvwmWindow *prev_fw = get_focus_window();
// END HACK


// printf("__handle_click_to_focus %d prev fw=%p new fw=%p\n", 
// __LINE__, 
// get_focus_window(),
// exc->w.fw);
// void dump_windows();
...
// BEGIN HACK
// move the previous window to the last position so mod-tab works
    swap_windows(prev_fw);
//printf("__handle_click_to_focus %d\n", __LINE__);
//dump_windows();
// END HACK
...
printf("HandleMapRequestKeepRaised %d\n", __LINE__);
void dump_windows();
dump_windows();
// BEGIN HACK
// store the previous focused window
...
// printf("dispatch_event %d type=%d\n", __LINE__, e->type);
