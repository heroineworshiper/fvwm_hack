...
// HACK
// poll alt/meta status for window circulating
static char *hack_display_name = 0;
volatile int mod_released = 0;
void* polling_hack()
{
    int mod_pressed = 0;
printf("polling_hack %d\n", __LINE__);
    Display *dpy2 = XOpenDisplay(hack_display_name);
printf("polling_hack %d dpy2=%p\n", __LINE__, dpy2);
    while(1)
    {
        XkbStateRec state_return;
        XkbGetState(dpy2, XkbUseCoreKbd, &state_return); 
//        printf("polling_hack %d %x\n", __LINE__, state_return.mods);
// detect mod release
#define THE_MOD 0x40
        int mod_pressed2 = ((state_return.mods & THE_MOD) == THE_MOD);
        if(!mod_pressed2 && mod_pressed) mod_released = 1;
        mod_pressed = mod_pressed2;
        usleep(50000);
    }
...
void init_polling_hack()
{
printf("init_polling_hack %d\n", __LINE__);
    pthread_attr_t  attr;
    pthread_t tid;
    pthread_attr_init(&attr);
    pthread_create(&tid, &attr, polling_hack, 0);
}
// END HACK
...
// HACK
        hack_display_name = display_name;
        init_polling_hack();
// END HACK
...
XSetWindowAttributes attr3;
attr3.event_mask = KeyPressMask | KeyReleaseMask;
XChangeWindowAttributes(dpy, Scr.Root, CWEventMask, &attr3);


XWindowAttributes attr2;
XGetWindowAttributes(dpy, Scr.Root, &attr2);
printf("main %d all_event_masks=%x your_event_mask=%x\n", 
__LINE__, attr2.all_event_masks, attr2.your_event_mask);
...
printf("main %d all_event_masks=%x your_event_mask=%x\n", 
__LINE__, attr2.all_event_masks, attr2.your_event_mask);
printf("main %d NoFocusWin=%p\n", __LINE__, Scr.NoFocusWin);
...
//printf("main %d SizeWindow=%p\n", __LINE__, Scr.SizeWindow);
	resize_geometry_window();
	initPanFrames();
	MyXGrabServer(dpy);
	checkPanFrames();
